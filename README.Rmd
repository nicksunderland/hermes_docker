...
**Code-ref:** [`line 148`](https://github.com/nicksunderland/hermes_docker/blob/main/run.sh#L148)
...
**Code-ref:** [`line 167`](https://github.com/nicksunderland/hermes_docker/blob/main/run.sh#L167)  
...
**Code-ref:** [`line 187`](https://github.com/nicksunderland/hermes_docker/blob/main/run.sh#L187)  

To ensure all datasets are meta-analysis ready, they are harmonized to **GRCh38**. This is because GRCh38 has the greatest coverage and results in the least amount of data loss. For example, nearly all of GRCh37 is represented in GRCh38, but the reverse is not true. This is simple to change should we want to continue with the original plan for a GRCh38 analysis.
...
* Effect Allele Frequency (EAF) comparison against reference (gnomAD/1kG).  
* P-Z plots.  
...
### 5. Resources  
Several resource files are required, including the reference fasta, dbSNP, and lift over chain files. To run this you will need to obtain the resource files. I'm trying to set up an AWS S3 bucket or similar so they are easy to download. 
